...
distance_and_mask_upper_bounds = np.concatenate((distance_upper_bounds, mask_upper_bounds, mask_upper_bounds),axis=0)
distance_and_mask_lower_bounds = np.concatenate((distance_lower_bounds, mask_lower_bounds, mask_lower_bounds),axis=0)
###########################
...
def cost_fun(x, grad):
    ...
    C1 = _compute_cost_1()
    C2 = _compute_cost_2()
    C = C1 + C2 #average cost function for both modes
    ...
    if grad.size > 0:
        if opt_cycle[0] < 1000:
            ...
            grad[:distance_num] = dist_grad_1() * (distance_upper_bound - distance_lower_bound) + dist_grad_2() * (distance_upper_bound - distance_lower_bound)
            #grad[:distance_num] = np.array([0,0,0]) #set distance gradient to zero for now
        grad[distance_num:2*S+distance_num] = adjoint_propagate_1() * 2 * np.pi 
        grad[2*S+distance_num:4*S+distance_num] = adjoint_propagate_2() * 2 * np.pi
...
if __name__ == "__main__":
    ...
    solver = nlopt.opt(nlopt.LD_CCSAQ, distance_num + 4*S)

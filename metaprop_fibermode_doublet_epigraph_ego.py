...
def make_1Dplot_of(x_axis,y_axis,legend_labels,plot_zoom_x=zoom_x,save_name="plot_1D",) -> None:
    ...
    for i in range(len(x_axis)):
        if len(legend_labels) != len(x_axis):
            raise ValueError("Length of legend_labels must match the number of plots.")
        else:
            plt.plot(x_axis[i], y_axis[i], label=legend_labels[i])
...
def epigraph_constraint(result,epigraph_and_weights,gradient)->None:
    ...
    Args:
        result: 1D array, modified in place, with the results of this constraint
            evaluation
        epigraph_and_weights: 1D array of epigraph (first element) and 
            design weights (remaining elements)
        gradient: gradients, modified in place, of the constraint function with respect to the design weights
    '''
    epigraph = epigraph_and_weights[0]
    weights = epigraph_and_weights[1:]

    obj_val, grad = value_and_grad(weights)
    obj_values1.append(obj_val[0])
    obj_values2.append(obj_val[1])
...
def obj_fun(epigraph_and_weights, grad):
    ...
    Args:
        epigraph_and_weights: 1D array of epigraph (first element) and 
            design weights (remaining elements)
        grad: gradient of the objective function with respect to the design weights, modified in place
    Returns:
...
w_norm = np.zeros((2*S)) #global definition concatenated normalized parameters for both masks
phase_mask = phase_given_w(w_norm) #global definition 
epigraph_and_weights = np.zeros((2*S + 1))
...
def metaprop_solve():
    ...
    solver = nlopt.opt(nlopt.LD_CCSAQ, 2*S + 1) #weights (2S) + epigraph (1)
    epigraph_and_weights_lower_bounds = np.insert(mask_lower_bounds.flatten(),0,-np.inf)
    epigraph_and_weights_upper_bounds = np.insert(mask_upper_bounds.flatten(),0,+np.inf)
    solver.set_lower_bounds(epigraph_and_weights_lower_bounds)
    solver.set_upper_bounds(epigraph_and_weights_upper_bounds)
    ...
    #verify the results
    opt_weights = epigraph_and_weights[1:]
...
def reset_globals():
    ...
    obj_values2[:] = []
    opt_history[:] = []
    w_norm[:] = np.zeros((2*S)) #reset the normalized phase masks for the next iteration
...
def metaprop(x):
    ...
    for i in range(m):
        ...
        with open('results/metaprop_results.txt', 'a') as f:
            f.write(f"{d[0]:.6e} {d[1]:.6e} {d[2]:.6e} {ob1:.6e} {ob2:.6e} {t:.6e}\n")
        
        alpha = 0.5
        C[i] = t + alpha * np.abs(ob1 - ob2) #cost function to be minimized by EGO)
...
def run_ego():
    ...
    ##############################
    n_ego_iter = 30
    xlimits = np.array([[200,700], [200,700], [200,700]])
    seed = 777

...
def cost_fun(x, grad):
    ...
    C1 = _compute_cost_1()
    C2 = _compute_cost_2()
    C = np.mean([C1, C2]) #average cost function for both modes
    ...
    if grad.size > 0:
        ...
        grad[distance_num:2*S+distance_num] = adjoint_propagate_1() * 2 * np.pi * 0.5
        grad[2*S+distance_num:4*S+distance_num] = adjoint_propagate_2() * 2 * np.pi * 0.5
...
if __name__ == "__main__":
    ...
    make_polarPlot_of(output_field1_2d, choose_quantity="amplitude", save_name="optimized_output_field_amplitude", plot_zoom_r=zoom_output_fiber_radius)
    make_polarPlot_of(output_field1_2d, choose_quantity="phase", save_name="optimized_output_field_phase", plot_zoom_r=zoom_output_fiber_radius)
    make_polarPlot_of(output_field2_2d, choose_quantity="amplitude", save_name="target_field_1_amplitude_polar", plot_zoom_r=zoom_output_fiber_radius)
    make_polarPlot_of(output_field2_2d, choose_quantity="phase", save_name="target_field_1_phase_polar", plot_zoom_r=zoom_output_fiber_radius)
